...
def ensure_csv_exists(filename):
    """Creates the gATH_table.csv with default data if it does not exist."""
    if os.path.exists(filename):
        return

    print(f"  [Info] '{filename}' not found. Creating with default data...")
    
    default_data = [
        ["Asset", "Type", "API_ID", "ATH_Value", "ATH_Date"],
        # CRYPTO
        ["BITCOIN",   "crypto", "bitcoin",   126198.10,  "10/05/2025"],
        ["ETHEREUM",  "crypto", "ethereum",  4953.73,    "08/24/2025"],
        ["SOLANA",    "crypto", "solana",    294.33,     "01/19/2025"],
        ["RIPPLE",    "crypto", "ripple",    3.84,       "01/13/2018"],
        ["CARDANO",   "crypto", "cardano",   3.10,       "09/02/2021"],
        ["DOGECOIN",  "crypto", "dogecoin",  0.7375,     "05/08/2021"],
        ["SHIBA-INU", "crypto", "shiba-inu", 0.00008845, "10/28/2021"],
        ["PEPE",      "crypto", "pepe",      0.00002824, "12/09/2024"],
        ["PAXG",      "crypto", "paxg",      2855.83,    "04/13/2024"],
        # STOCKS
        ["NVDA", "stock", "NVDA", 235.74,  "05/14/2026"],
        ["AMD",  "stock", "AMD",  584.73,  "06/30/2026"],
        ["INTC", "stock", "INTC", 142.35,  "06/30/2026"],
        ["MSTR", "stock", "MSTR", 457.22,  "01/01/2025"],
        ["ORCL", "stock", "ORCL", 345.72,  "09/10/2025"],
        ["AI",   "stock", "AI",   30.11,   "01/01/2025"],
        ["GTLB", "stock", "GTLB", 52.38,   "11/03/2025"],
        ["TEM",  "stock", "TEM",  104.32,  "11/09/2025"],
        ["CGNX", "stock", "CGNX", 101.82,  "02/12/2021"],
        ["SNOW", "stock", "SNOW", 280.16,  "06/01/2026"],
        ["DDOG", "stock", "DDOG", 278.70,  "06/01/2026"],
        ["IPGP", "stock", "IPGP", 155.82,  "01/01/2026"],
        ["MU",   "stock", "MU",   1213.56, "06/25/2026"],
        ["ASX",  "stock", "ASX",  45.32,   "06/30/2026"],
        ["AMKR", "stock", "AMKR", 96.29,   "06/22/2026"],
        ["ANET", "stock", "ANET", 179.80,  "01/01/2026"],
        ["WDC",  "stock", "WDC",  799.87,  "06/18/2026"],
        ["AVGO", "stock", "AVGO", 495.00,  "06/03/2026"],
        ["MRVL", "stock", "MRVL", 329.88,  "06/18/2026"],
        ["KEYS", "stock", "KEYS", 373.34,  "06/22/2026"],
        ["SNDK", "stock", "SNDK", 2348.00, "06/25/2026"],
        ["CSCO", "stock", "CSCO", 130.37,  "06/04/2026"],
        ["QCOM", "stock", "QCOM", 259.92,  "05/29/2026"],
        ["ARM",  "stock", "ARM",  439.46,  "06/18/2026"]
    ]
...
def load_csv_config(filename):
    """Reads the CSV file and returns a dictionary of assets."""
    config = {}
    with open(filename, mode='r', newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            asset_name = row["Asset"].strip().upper()
            try:
                ath_val = float(row["ATH_Value"])
            except ValueError:
                ath_val = 0.0
            ...
            config[asset_name] = {
                ...
                "ath": ath_val,
                "date": row["ATH_Date"].strip()
            }
    return config

def save_csv_config(filename, config):
    """Saves the updated configuration dictionary back to the CSV file."""
    with open(filename, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Asset", "Type", "API_ID", "ATH_Value", "ATH_Date"])
        for asset, data in config.items():
            # Format numbers to avoid scientific notation for small cryptos
            if data['ath'] < 0.01:
                ath_str = f"{data['ath']:.8f}"
            else:
                ath_str = f"{data['ath']:.2f}"
            writer.writerow([asset, data["type"], data["api_id"], ath_str, data["date"]])
    print(f"  ✔ Updated ATH table saved to '{filename}'")
...
def process_raw_data(name, prices, current_price,
    ...
    try:
        ...
        # Pull updated ATH from the loaded CSV configuration
        ath_val  = asset_config.get(name, {}).get("ath",  "N/A")
        ath_date = asset_config.get(name, {}).get("date", "N/A")
...
def apply_asset_formatting(ws, current_row, r, headers,
    ...
    # --- Propagate daily cell format (red/blue) to weekly/monthly/tracking cells ---
    daily_match_formats = []
    for i, price in enumerate(r['daily']):
        if price != "" and isinstance(price, (int, float)):
            is_bot = (r['min_month12'] != "" and price <= r['min_month12'] * 1.05)
    ...
    # --- Match tracking formats to weekly/monthly/daily cells ---
    tracking_formats = []
...
def write_sheet_row(ws, results):
    ...
    # Strict 8-character aRGB hex colors required by Excel OOXML standard
...
def fetch_crypto_data(asset_config, previous_counts, previous_tracking_data):
    print("\n" + "=" * 65)
    ...
    for name, config in asset_config.items():
        ...
        coin_api_id = config["api_id"]
        url    = f"https://api.coingecko.com/api/v3/coins/{coin_api_id}/market_chart"
        params = {"vs_currency": "usd", "days": "365"}

        for attempt in range(3):
            try:
                print(f"  Fetching {name:<12} ... ", end="", flush=True)
                ...
                # Extract prices and track new ATH if found
                for ts, pr in data['prices']:
                    prices.append(pr)
                    if pr > max_price:
                        max_price = pr
                        # Convert ms timestamp to date string
                        max_date = datetime.fromtimestamp(ts / 1000.0).strftime("%m/%d/%Y")

                # Dynamic ATH update in memory (will be saved to CSV later)
                if max_price > config["ath"]:
                    print(f"[NEW ATH: {max_price:.4f} on {max_date}] ", end="")
                    asset_config[name]["ath"] = max_price
                    asset_config[name]["date"] = max_date

                res = process_raw_data(
                    name, prices, prices[-1],
                    previous_counts, previous_tracking_data, asset_config, is_stock=False)
                if res:
                    results.append(res)
                    print("OK")
                time.sleep(5)
...
def fetch_stock_data(asset_config, previous_counts, previous_tracking_data):
    print("\n" + "=" * 65)
    ...
    for name, config in asset_config.items():
        ...
        ticker_id = config["api_id"]
        try:
            print(f"  Fetching {name:<6} ... ", end="", flush=True)
            ...
            # Extract highest price from the fetched history
            for d, pr in zip(dates, prices):
                if pr > max_price:
                    max_price = pr
                    max_date = d
            
            # Also check if the current live price is the new ATH
            ...
            # Dynamic ATH update in memory (will be saved to CSV later)
            if max_price > config["ath"]:
                print(f"[NEW ATH: {max_price:.2f} on {max_date}] ", end="")
                asset_config[name]["ath"] = max_price
                asset_config[name]["date"] = max_date

            res = process_raw_data(
                name, prices, current_price,
                previous_counts, previous_tracking_data, asset_config, is_stock=True)
            if res:
                results.append(res)
...
def fetch_all_data():
    # 1. Setup and load configuration from CSV
    ensure_csv_exists(ATH_CSV_FILE)
    asset_config = load_csv_config(ATH_CSV_FILE)

    # 2. Read previous data from both sheets of the combined report
    ...
    # 3. Fetch Data (will update asset_config in memory if new ATHs are found)
    crypto_results = fetch_crypto_data(asset_config, previous_counts, previous_tracking)
    stock_results  = fetch_stock_data(asset_config, previous_counts,  previous_tracking)
    all_results    = crypto_results + stock_results

    # 4. Save updated ATH data back to CSV
    print("\n" + "=" * 65)
    print("  Updating ATH CSV Table ...")
    print("=" * 65)
    save_csv_config(ATH_CSV_FILE, asset_config)

    # 5. Print summaries and write Excel
    ...
    print(f"  ATH Table      : {ATH_CSV_FILE}")
    print(f"  Sheet 1        : '{SHEET_ROW}'  (assets = rows)")
    print(f"  Sheet 2        : '{SHEET_COL}'  (assets = columns)")
    print(f"  Crypto total   : {len(crypto_results)}")
    print(f"  Stocks total   : {len(stock_results)}")
